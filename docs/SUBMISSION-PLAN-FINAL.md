...
## 5.5 【審査前に必須】Dashboard のセッション切れUXを直す（2026-05-29発見）

**症状**: ログインから約1時間でNextAuthトークンが切れると `/orders` が
`Application error: a server-side exception has occurred`（digest 3304361206）で白画面クラッシュ。
シークレットウィンドウ（クリーンセッション）では正常表示される。

**原因**: コード/env/Functions は正常。**セッション/トークン期限切れ時のハンドリングが無い**こと。
Functions が 401 を返す → `/orders` のサーバーコンポーネントの `listOrders()` fetch が例外を throw → クラッシュ。

**なぜ必須か**: 審査員は 6/2〜18 の任意タイミングで触る＝**必ずトークン切れ状態で来る**。
そのとき白画面が出ると完成度・実現性（軸③）で致命的減点。「再サインインで直る」では済まない。

**修正（別セッションの実装タスク）**:
1. `packages/dashboard` の `/orders` と `/orders/[id]` で、セッション/トークンが無効なら
   `redirect()` でサインインページへ誘導（白画面で落とさない）。
2. `app/orders/error.tsx`（or 上位）のエラーバウンダリで「読み込みに失敗しました→再サインイン」+ 再試行ボタン。
3. できれば NextAuth の refresh token で自動更新（難度高。最低限 1+2 で「落ちない」を担保すれば審査は守れる）。

今日のセッションはコンテキストが厚いので、**別のクリーンセッションで腰を据えて実施**するのが安全。
