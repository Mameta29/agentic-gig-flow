...
- **Step 2・3・4・7 完了 + Container Apps デプロイ完了。** 残りは Step 5/6（ユーザー作業）と E2E。
...
- Step 4（JPYC 送金疎通）Amoy testnet で完了（§9）。
- **Container Apps 稼働中**（§12）: Dashboard・MCP サーバとも Container App としてデプロイ・HTTP 応答確認済み。
- **次は Step 5（Copilot Studio）/ Step 6（Fabric）— ブラウザ UI 操作のためユーザー作業必須。**

### Container Apps の確定値（§12）

| サービス | URL |
|---|---|
| Dashboard | `https://ca-gigflow-dashboard.mangomeadow-46aa4d19.japaneast.azurecontainerapps.io` |
| MCP サーバ | `https://ca-gigflow-mcp.mangomeadow-46aa4d19.japaneast.azurecontainerapps.io` |

- 両方 `minReplicas 1`（デモのコールドスタート回避）。GHCR からイメージ pull（public 化済み）。
- MCP: `/healthz` 200 / `/mcp` POST は Entra 401 ガード動作確認済み。
- Dashboard: root 200。Entra SSO 用 redirect URI 登録済み。
...
| 7 | GitHub demo repo + Webhook + Functions + Container Apps デプロイ | ✅ 完了（§12）|
| 8 | E2E リハーサル → Workbook デプロイ → 動画 → Zenn → 提出（6/1締切）| 🟡 E2E は通った（§13）。残りは動画/記事/提出 |

### E2E 通し完了（2026-05-23・§13）

PR マージ → Review(gpt-4o) → 自動 approve+merge → Settlement → **Amoy で 100 JPYC 着金** →
Bookkeeping(仕訳生成) → status `bookkept` まで本番環境で完走を確認した。
送金 tx `0x4c22fc096eacbe00a8cd35e9fadc714ab3bb5eb4df9c885a4f6f722ce988e263`（block 38845402）。
検証で見つけた**2つの実バグ**と**1つの未解決レース**は §13 を参照。
...
## 12. Container Apps デプロイ顛末（Dashboard / MCP）

### 構成

- Container Apps Env `cae-gigflow-28fa80` 上に `ca-gigflow-dashboard`（port 3000）/
  `ca-gigflow-mcp`（port 3333）を作成。両方 external ingress、`minReplicas 1`。
- イメージは GHCR（`ghcr.io/mameta29/gigflow-{dashboard,mcp}`）。`deploy-containers.yml` が
  ビルド & プッシュ & `az containerapp update` する。
- Dashboard の env: `AUTH_ENTRA_CLIENT_ID/SECRET`・`AUTH_SECRET`・`AUTH_URL`・`AUTH_TRUST_HOST`・
  `FUNCTIONS_APP_ID`・`MCP_APP_ID`・`FUNCTIONS_BASE_URL`・`POLYGON_CHAIN_ID`。
- MCP の env: `COSMOS_ENDPOINT`・`COSMOS_DATABASE`・`MCP_APP_AUDIENCE`・`PORT`。
- Dashboard 用 Entra クライアントシークレットを `app-gigflow-dashboard` に作成し、Key Vault に
  `dashboard-client-secret`、`AUTH_SECRET` を `dashboard-auth-secret` として保存（参考用バックアップ。
  実値は Container App の env に直接設定済み）。
- `app-gigflow-dashboard` の redirect URI に Container App の URL を登録済み。

### ハマったところ

1. `deploy-containers.yml` が `github.repository_owner`（`Mameta29`、大文字）を GHCR タグに
   使い、「repository name must be lowercase」で失敗 → owner を小文字化するステップを追加（コミット `7c81a44`）。
2. GHCR にプッシュされたイメージは初期 private で Container App が pull できず
   `UNAUTHORIZED` → **GHCR パッケージ2つを public 化**（ユーザーが GitHub UI で実施）。
3. MCP コンテナが `ERR_MODULE_NOT_FOUND: @hono/node-server` で起動失敗。Dockerfile の
   runner ステージがルートの `node_modules` だけコピーしており、pnpm のシンボリックリンク
   ワークスペース構造では各パッケージの依存が解決できなかった → **`pnpm deploy` 方式に変更**
   して自己完結ディレクトリを作る（コミット `42b48b8`）。Dashboard は Next.js standalone 出力
   なので元から問題なし。

### 残 TODO

- Dashboard イメージは sha `7c81a44`（Dockerfile 修正前）で動作中。実害はないが、次に
  `packages/dashboard/**` を変更すれば最新で再ビルドされる。
- Dashboard の Entra SSO は redirect URI 登録済みだが、実ログインの E2E 確認は未
  （Step 8 のリハーサルで行う）。
- MCP の `/mcp` は Entra トークン必須。Claude Desktop 等から繋ぐ設定は `docs/09-mcp-server.md` 参照。

---

## 13. E2E 通し検証の顛末（2026-05-23）

PR マージ → 報酬着金の本番フローを demo repo `Mameta29/gigflow-demo-workspace` で実通しした記録。

### 使ったもの

- 検証 order: `seed-e2e-order.ts`（orderId `adae02bc-f748-4c4b-8da4-d60f6ca7be82`、
  repo `Mameta29/gigflow-demo-workspace`、worker `ei-chan-bot`、受取 `0x7F37f6D0c5B4D41E3722d12930430FE309489389`、
  100 JPYC、status を最初から `in_progress` で投入＝`in_progress→pr_opened→review_passed` を許可遷移にするため）。
- Worker は別アカウント `ei-chan-bot`（demo repo に write collaborator として招待・承認済み）。
  PR は Web UI から作成、Review Agent(`Mameta29` PAT)が approve→merge する「人間 Worker × AI レビュアー」構成。
- Issue #1（orderId マーカー埋込）、PR #2。

### 見つけて直した実バグ（コミット `96d4aff`）

1. **CI レース → Review が誤 reject**。`pull_request synchronize` Webhook は push 直後
   （新コミットの check-run が未作成 or in_progress）に届くため、`getCheckRunStatus` が
   `pending` を返し、`review.ts` の「ciStatus !== success なら reject」ルールで合格 PR を落としていた。
   → `github.ts` に `waitForCheckRun`（pending の間だけ最大20秒ポーリング）を追加し、
   `github-webhook.ts` を差し替え。効果実証: review_started が webhook 受信から ~9秒遅延し、
   qualityScore が 0→90 に改善した。**本番デモのタイミング競合もこれで恒久解消。**
2. **PAT のリポジトリスコープ違い**。Key Vault `github-pat` が `Mameta29/agentic-gig-flow`
   にしか権限を持たず、`gigflow-demo-workspace` の PR review/merge が 403。gpt-4o は
   criteria 全 met・qualityScore 90 を出しつつ「GitHub API 認証エラーでマージ不可」として
   reject に切替えていた（reviewComment にそう書いてあった＝§13 で review.ts に
...
- merge を起こした synchronize の Review 処理（order を `review_passed` に遷移）と、
  ...
  closed 側が `getOrder` した時点でまだ `review_passed` になっておらず、`runSettlement` の
  `status !== 'review_passed'` で空振り（`settlement_started` すら出ない／202 で握り潰し）。
  今回は `run-settlement-e2e.ts` で `runSettlement`+`runBookkeeping` を手動起動して送金を完走させた。
...
- 送金 tx: `0x4c22fc096eacbe00a8cd35e9fadc714ab3bb5eb4df9c885a4f6f722ce988e263`（Amoy block 38845402、status success、100 JPYC）。
- 最終 order status: `bookkept`（review_passed → settled → bookkept）。
