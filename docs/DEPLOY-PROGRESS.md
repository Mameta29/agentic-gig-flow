...
Step 4（**Polygon Amoy testnet で実施**。詳細・コマンドは §9）—
1. デモ専用ウォレットの秘密鍵を生成（チャットに貼らず `! ` 実行でファイルに）
2. Key Vault `kv-gigflow-28fa80` に secret `wallet-pk` として保存
3. ウォレットに POL/MATIC（faucet）+ JPYC（Amoy）を入金
4. 完了後、`scripts/test-transfer.ts` で送金疎通（私が実行可）

Function App は既に Amoy 設定済み（`POLYGON_RPC` / `POLYGON_CHAIN_ID=80002`）。
コードも network 切替対応済み・デプロイ済み（コミット `307838d`）。
